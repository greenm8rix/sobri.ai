...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, sender, timestamp }) => {
  ...
  const escapeHtml = (unsafe: string): string => {
    return unsafe
      ...
      .replace(/>/g, "&gt;")  // Attempting double escape for >
      .replace(/"/g, "&quot;") // Attempting double escape for "
      .replace(/'/g, "&#039;").replace(/'/g, "&#039;"); // Attempting double escape for ' (or use &#039; directly if & is the issue)
...
// Process AI messages: escape HTML first, then apply markdown for bold
const aiMessageHtml = isUser
  ? ''
  : escapeHtml(message).replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
